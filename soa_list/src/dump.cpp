...
void list_dump (unsigned int error_code)
{
    ...
    for (int i = 0; i < graph_count; i++)
    {
        ...
        for (int index = 0; index < graph_calls[i].list_cpy->capacity; index++)
            fprintf(html_file, " %4" PRINT_SPEC, graph_calls[i].list_cpy->data[index]);    
        fprintf(html_file, "\n");
...
void make_table(spisok_t* list, FILE* file, int index, const char* type)
{
    ...
    {
    fprintf(file, "%s%d [label=\"{{index: %d}|{next: %d|prev: %d}|{value: %" PRINT_SPEC "}}\"", type, index, index, list->next[index], list->prev[index], list->data[index]);
