...
void list_dump(unsigned int error_code) {
  ...
  if (error_code != LIST_NO_ERRORS) {
    ...
    if (error_code & LIST_BAD_DATA_PTR)
      fprintf(html_file,
              "<li>LIST_BAD_DATA_PTR - Invalid data array pointer</li>\n");
    if (error_code & LIST_BAD_NEXT_PTR)
      fprintf(html_file,
              "<li>LIST_BAD_NEXT_PTR - Invalid next array pointer</li>\n");
    if (error_code & LIST_BAD_PREV_PTR)
      fprintf(html_file,
              "<li>LIST_BAD_PREV_PTR - Invalid prev array pointer</li>\n");
    if (error_code & LIST_BAD_ANCHOR_ELEM)
      fprintf(html_file,
              "<li>LIST_BAD_ANCHOR_ELEM - Invalid anchor element</li>\n");
    if (error_code & LIST_NEXT_CYCLED)
      fprintf(html_file,
              "<li>LIST_NEXT_CYCLED - Cycle detected in next pointers</li>\n");
    if (error_code & LIST_PREV_CYCLED)
      fprintf(html_file,
              "<li>LIST_PREV_CYCLED - Cycle detected in prev pointers</li>\n");
    if (error_code & LIST_NEXT_NOT_EQV_PREV)
      fprintf(html_file, "<li>LIST_NEXT_NOT_EQV_PREV - Next and prev pointers "
                         "are not eqvivalent</li>\n");
  ...
  for (int i = 0; i < graph_count; i++) {
    if (graph_calls[i].filename != NULL)
      fprintf(html_file, "<h3>%s %s() in %s:%d</h3>\n",
              graph_calls[i].call_position, graph_calls[i].function,
              graph_calls[i].filename, graph_calls[i].line);
    else
      fprintf(html_file, "<h3>Emergensy exit</h3>\n");

    fprintf(html_file, "%6s", "Index:");
    for (int index = 0; index < graph_calls[i].list_cpy->capacity; index++)
      fprintf(html_file, " %4d", index);
    fprintf(html_file, "\n");

    fprintf(html_file, "%6s", "Data:");
    for (int index = 0; index < graph_calls[i].list_cpy->capacity; index++)
      fprintf(html_file, " %4" PRINT_SPEC,
              graph_calls[i].list_cpy->data[index]);
    fprintf(html_file, "\n");

    fprintf(html_file, "%6s", "Next:");
    for (int index = 0; index < graph_calls[i].list_cpy->capacity; index++)
      fprintf(html_file, " %4d", graph_calls[i].list_cpy->next[index]);
    fprintf(html_file, "\n");

    fprintf(html_file, "%6s", "Prev:");
    for (int index = 0; index < graph_calls[i].list_cpy->capacity; index++)
      fprintf(html_file, " %4d", graph_calls[i].list_cpy->prev[index]);
    fprintf(html_file, "\n");
  ...
  char command[STR_MAX_LEN + 10] = "";
  sprintf(command, "xdg-open %s", html_filename);
...
void list_info_copy(spisok_t *list, spisok_t *list_cpy) {
  ...
  list_cpy->data = (list_t *)calloc(list->capacity, sizeof(list_t));
...
void list_graph(spisok_t *list, const char *filename) {
  ...
  fprintf(file, "node [shape=diamond, margin=0.1, width=1, style=filled, "
                "fillcolor=red];\n");
  for (int i = 0; i < list->capacity; i++) {
    if (list->prev[i] != POIZON)
...
void make_table(spisok_t *list, FILE *file, int index, const char *type) {
  if (strcmp(type, "used") == 0) {
    fprintf(
        file,
        "%s%d [label=\"{{index: %d}|{next: %d|prev: %d}|{value: %" PRINT_SPEC
        "}}\"",
        type, index, index, list->next[index], list->prev[index],
        list->data[index]);
    fprintf(file, "shape=Mrecord, fillcolor=lightblue]\n");
  }

  else {
    fprintf(file, "%s%d [label=\"{{index: %d}|{FREE}|{next: %d}}\",", type,
            index, index, list->next[index]);
...
void make_arrows(const spisok_t *list, FILE *file) {
  ...
  for (int i = 0; i < list->capacity; i++) {
    if (list->prev[i] != POIZON && list->prev[i] != 0 &&
        list->prev[i] != POIZON)
  ...
  while (free_head != POIZON && free_head < list->capacity) {
    if (list->next[free_head] != POIZON &&
        list->next[free_head] < list->capacity) {
      fprintf(
          file,
          "free%d -> free%d [color=green, style=dotted, constraint=false];\n",
...
void make_graph(const char *filename) {
  ...
  sprintf(programm, "dot %s -T png -o TEST%d.png", filename, graph_count++);
